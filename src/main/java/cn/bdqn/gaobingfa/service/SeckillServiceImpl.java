...
import cn.bdqn.gaobingfa.Util.SellException;
import cn.bdqn.gaobingfa.dao.ProductStockDao;
...
public class SeckillServiceImpl implements SeckillService {
  ...
  /*  public SeckillServiceImpl() {
    ...
    public SeckillServiceImpl(RedisLock1 redisLock) {
    ...
    static {
        ...
        stock = new HashMap<>();
        orders = new HashMap<>();
       /* products.put("123456",50);
        stock.put("123456",50);*/
    }

    private String queryMap(String productId){//模拟查询数据库
        ProductStock productStock=productStockDao.findByProductId(productId);
        /*products.put(productId,productStock==null?0:productStock.getProducts());
        stock.put(productId,productStock.getStock());*/
       ...
       /* return "国庆活动，皮蛋特教，限量"
                ...
                +orders.size()+"人";*/
        return "国庆活动，皮蛋特教，限量"
                +productStock.getProducts()
                +"份,还剩:"+productStock.getStock()
                +"份,该商品成功下单用户数:"
                +productStock.getOrders()+"人";
    }
    ...
    public void orderProductMocckDiffUser(String productId) {//解决方法一:synchronized锁方法是可以解决的，但是请求会变慢,请求变慢是正常的。主要是没做到细粒度控制。比如有很多商品的秒杀，但是这个把所有商品的秒杀都锁住了。而且这个只适合单机的情况，不适合集群
      ...
      //  int stockNum = stock.get(productId);
        int stockNum = productStock.getStock();
        if(stockNum==0){
            throw new SellException(100,"活动结束");
        }else {
            //2.下单
            orders.put(System.currentTimeMillis()+"1119",productId);
            //3.减库存
          //  stockNum =stockNum-1;//不做处理的话，高并发下会出现超卖的情况，下单数，大于减库存的情况。虽然这里减了，但由于并发，减的库存还没存到map中去。新的并发拿到的是原来的库存
           if (productStockDao.update(productId)>0){
               stockNum =stockNum-1;
